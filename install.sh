...
write_compose_file() {
  ...
  local monitoring_depends_on=""
  local monitoring_env=""
  local monitoring_services=""
  local monitoring_network_definition=""
  local monitoring_volume_definition=""

  if [ "$INSTALL_MONITORING" = "true" ]; then
    monitoring_depends_on=$'      prometheus:\n        condition: service_started'
    monitoring_env='      PROMETHEUS_URL: http://prometheus:9090'
    monitoring_networks=$'      - monitoring'
    monitoring_volume_definition=$'  prometheus_data:\n'
    monitoring_network_definition=$'  monitoring:\n'
    monitoring_services=$(cat <<'YAML'
...
volumes:
  dockroot_data:
  postgres_data:
${monitoring_volume_definition}

networks:
  frontend:
  backend:
${monitoring_network_definition}
YAML
}
...
main() {
  ...
  if [ "$INSTALL_MODE" = "custom" ]; then
    ...
    info "Monitoring: ${INSTALL_MONITORING}"
  fi
