...
import type { TrackedStackRow } from "@/components/stacks-table-workspace/types";
import { LinkButton } from "@/components/ui/link-button";

export function StacksLiveConsoleDock({
	stack,
	environmentId,
	onClose,
}: {
	stack: TrackedStackRow;
		...
		<div className="fixed inset-y-4 right-4 z-40 w-[min(44rem,92vw)] max-w-xl rounded-xl border border-default/12 bg-surface/95 shadow-[var(--shadow-lg)] backdrop-blur-sm">
			<div className="flex items-center justify-between border-b border-default/10 px-4 py-3">
				...
				<div className="flex items-center gap-1.5">
					<LinkButton
						href={`/dashboard/stacks/${stack.stackId}${environmentId ? `?environment=${environmentId}` : ""}`}
						variant="ghost"
						size="icon-xs"
			...
			<div className="p-3">
				<LiveStackFeed
					...
					initialLog={stack.lastDeployment?.log}
